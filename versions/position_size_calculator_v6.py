...
class FPSCApp(tk.Tk):
    def __init__(self):
        ...
        # Minimum Risk Label (new feature)
        self.min_risk_label = tk.Label(self, text="Min risk to trade: -", fg="grey")
        self.min_risk_label.grid(row=2, column=1, columnspan=3, sticky='w')

        # Stop Loss
        tk.Label(self, text="Stop Loss (Ticks):").grid(row=3, column=0, sticky='e')
        self.stop_entry = tk.Entry(self, textvariable=self.stop_var, width=12)
        self.stop_entry.grid(row=3, column=1, sticky='w')

        # Contract Type (Mini/Micro, etc.)
        tk.Label(self, text="Contract Type:").grid(row=4, column=0, sticky='e')
        ...
        self.contract_combo.grid(row=4, column=1, sticky='w')
        ...
        tk.Label(self, text="Instrument:").grid(row=5, column=0, sticky='e')
        ...
        self.instrument_search.grid(row=5, column=1, sticky='w')

        self.instrument_combo = ttk.Combobox(self, textvariable=self.instrument_var, state='readonly', width=32)
        self.instrument_combo.grid(row=5, column=2, columnspan=2, sticky='w')
        ...
        tk.Label(self, text="Tick Size:").grid(row=6, column=0, sticky='e')
        self.tick_size_entry = tk.Entry(self, textvariable=self.tick_size_var, state='readonly', width=12)
        self.tick_size_entry.grid(row=6, column=1, sticky='w')

        tk.Label(self, text="Tick Value:").grid(row=6, column=2, sticky='e')
        self.tick_value_entry = tk.Entry(self, textvariable=self.tick_value_var, state='readonly', width=12)
        self.tick_value_entry.grid(row=6, column=3, sticky='w')

        tk.Label(self, text="Currency:").grid(row=7, column=0, sticky='e')
        self.currency_entry = tk.Entry(self, textvariable=self.currency_var, state='readonly', width=12)
        self.currency_entry.grid(row=7, column=1, sticky='w')
        ...
        for var in [self.account_var, self.risk_var, self.stop_var, self.contract_type_var, self.instrument_var, self.risk_mode]:
            var.trace_add('write', lambda *a, **kw: self.calculate())
    ...
    def update_instrument_dropdown(self, *args):
        ...
        if names:
            ...
            self.min_risk_label.config(text="Min risk to trade: -", fg="grey")
    ...
    def instrument_selected(self, event=None):
        ...
        if 0 <= idx < len(self.instrument_search_results):
            ...
            # Calculate and show minimum risk to trade
            try:
                tick_value = float(inst["tick_value"])
                tick_size = float(inst["tick_size"])
                min_risk_required = tick_value / tick_size if tick_size else 0
                self.min_risk_label.config(text=f"Min risk to trade: ${min_risk_required:.2f}", fg="grey")
            except Exception:
                self.min_risk_label.config(text="Min risk to trade: -", fg="grey")
        else:
            self.tick_size_var.set("")
    ...
    def calculate(self, *args):
        ...
        try:
            ...
            if risk <= 0:
                self.error_var.set("Risk must be positive.")
                self.highlight_entry(self.risk_entry, True)
                error = True
            ...
            # Minimum risk check (new feature)
            min_risk_required = tick_value / tick_size if tick_size else 0
            self.min_risk_label.config(text=f"Min risk to trade: ${min_risk_required:.2f}", fg="grey")
            risk_dollars = risk if mode == "dollars" else (account_size * risk / 100.0)
            if risk_dollars < min_risk_required:
